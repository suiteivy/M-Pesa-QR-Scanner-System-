    ...
    <div className="min-h-screen bg-brand-light dark:bg-brand-black text-content-main dark:text-content-mainDark transition-colors duration-300">
      ...
      {!isDemoExpired && timeLeft && (
        <div className="bg-brand-orange text-white px-4 py-3 text-center flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6 shadow-md z-50 relative animate-in slide-in-from-top-4">
          ...
          <button
            ...
            className="bg-brand-buttonBase dark:bg-brand-buttonDark text-brand-buttonText dark:text-brand-buttonTextDark px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl flex items-center gap-2"
          >
      ...
      {isDemoExpired && (
        <div className="bg-status-errorBg text-status-error border-b border-status-error/20 px-4 py-3 text-center flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6 shadow-md z-40 relative animate-in slide-in-from-top-4">
          ...
          <button
            onClick={() => setShowBookingModal(true)}
            className="bg-status-error text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-colors shadow-sm flex items-center gap-2"
          >
      ...
      {/* HEADER */}
      <header className="bg-brand-surface dark:bg-brand-gray border-b border-brand-borderLight dark:border-brand-borderDark sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 md:h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[10px] text-content-muted dark:text-content-mutedDark font-bold uppercase tracking-widest mt-0.5 truncate max-w-[150px]">
                Interactive Sandbox
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:hidden">
            <ThemeToggle />
            <Button variant="outline" size="icon" onClick={() => setShowNavMenu(!showNavMenu)} className="border-brand-borderLight dark:border-brand-borderDark relative z-50">
      ...
      <main className={`max-w-7xl mx-auto px-4 py-6 sm:px-6 sm:py-8 space-y-8 pb-32 transition-opacity ${isDemoExpired ? 'opacity-30 pointer-events-none blur-sm' : 'opacity-100'}`}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="flex w-full overflow-x-auto bg-brand-light dark:bg-brand-black p-1.5 rounded-2xl border border-brand-borderLight dark:border-brand-borderDark no-scrollbar">
            <TabsTrigger value="overview" className="flex-1 py-3 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-brand-surface dark:data-[state=active]:bg-brand-gray rounded-xl shadow-sm transition-all">Overview</TabsTrigger>
            <TabsTrigger value="transactions" className="flex-1 py-3 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-brand-surface dark:data-[state=active]:bg-brand-gray rounded-xl shadow-sm transition-all">Ledger</TabsTrigger>
            <TabsTrigger value="menu" className="flex-1 py-3 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-brand-surface dark:data-[state=active]:bg-brand-gray rounded-xl shadow-sm transition-all">Setup Menu</TabsTrigger>
          </TabsList>
   ...
   <AnalyticsModule analytics={analytics} />
          ...
          <TabsContent value="transactions" className="mt-6">
            <Card className="bg-brand-surface dark:bg-brand-gray border-brand-borderLight dark:border-brand-borderDark shadow-sm rounded-2xl overflow-hidden">
              <CardHeader className="p-5 border-b border-brand-borderLight dark:border-brand-borderDark bg-brand-light dark:bg-brand-black/50">
                ...
                {transactions && transactions.length > 0 ? (
                  <div className="divide-y divide-brand-borderLight dark:divide-brand-borderDark">
                    {transactions.slice(0, 10).map((transaction, idx) => {
                      ...
                      return (
                        <div key={transaction.id || idx} className="p-4 sm:p-5 flex items-center justify-between hover:bg-brand-light dark:hover:bg-brand-black/30 transition-colors">
                          <div className="flex items-center gap-4">
                            <div className={`p-2.5 rounded-xl ${isSuccess ? 'bg-status-successBg text-status-success' : 'bg-brand-light text-content-muted dark:bg-brand-black'}`}>
                              <CreditCard className="w-5 h-5" />
                            </div>
                            <div>
                              <p className="font-bold text-sm text-content-main dark:text-content-mainDark truncate">
                                {transaction.phoneNumber || transaction.accountRef || 'Walk-in Customer'}
                              </p>
                              <p className="text-[10px] text-content-muted uppercase tracking-widest font-semibold mt-0.5">
                                {formatDate(transaction.createdAt)}
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="font-black text-sm sm:text-base text-content-main dark:text-content-mainDark">
                  ...
                  <div className="text-center py-20 px-6">
                    <div className="bg-brand-light dark:bg-brand-black w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 border border-brand-borderLight dark:border-brand-borderDark">
                      <QrCode className="w-8 h-8 text-content-muted" />
                    </div>
                    <h3 className="text-sm font-black text-content-main dark:text-content-mainDark uppercase tracking-widest mb-2">No Transactions Yet</h3>
                    <p className="text-xs text-content-muted font-medium mb-6 max-w-[250px] mx-auto">
